 ...
 for i in sys.argv:
  if i.startswith('-'):
   ...
   if option=="v":
    ...
    sys.exit()
   if option=="orb":
      orbital = sys.argv[sys.argv.index('-orb')+1]
   if option=="h":
    print('''
    ...
    -orb set orbitals to sum: total, pz, py, px, dz2, dxz, dyz, dx2y2, dxy
    -h print this help
    -v print version
...
if len(os.popen('echo $DISPLAY').read()) > 1 or len(os.popen('echo $DISPLAY').read()) <= 1:
  graphic_plot="yes"
  try:
   from pylab import *
   mplplot="yes"
...
if orbital == 'total':
	column = 1
elif orbital == 'pz' or orbital == 'dz2':
	column = 3
elif orbital == 'px' or orbital == 'dzx' or orbital == 'dxz':
	column = 5
elif orbital == 'py' or orbital == 'dzy' or orbital == 'dyz':
	column = 7
elif orbital == 'dx2y2':
	column = 9
elif orbital == 'dxy':
...
for i in range(len(dosfiles)):
 ...
 for line in open(dosfiles[i],'r'):
  if len(line) > 10 and line.split()[0] != "#":
   ...
   if ksolved=="no":
      mati.append([float(line.split()[0]),float(line.split()[column]),float(line.split()[column+1])])
...
if graphic_plot=="yes":
  ...
  if mplplot=="yes":
    ...
    if min_y != "" and max_y != "":
       plt.axis([fromx, tox, min_y,max_y])
    else:
       plt.axis([fromx, tox, min(y2),max(y1)])
    plt.savefig('larruzo.png')
    #print(min_y, max_y)
    show()
